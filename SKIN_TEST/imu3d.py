...
def dead_reckon(path):
    time, aX, aY, aZ, wX, wY, wZ, mX, mY, mZ = old_data_rectangle(path)
    # time = time_array
    # aX = acc_array[:,0]
    # aY = acc_array[:,1]
    # aZ = acc_array[:,2]
...
def old_data_rectangle(path):
    acc_path = 'Data/' + path + '/Accelerometer.csv'
    omega_path = 'Data/' + path + '/Gyroscope.csv'
    mag_path = 'Data/' + path + '/MagneticField.csv'
    f = open(acc_path, 'r')
...
def pull_and_sort_data(path):
    self_path = 'Data/Self/' + path + '.csv'
    df = pd.read_csv(self_path, sep='\s*,\s*',
...
def grab_data_from_df(self_data, path):
    if self_data == False:
        time_acc, aX, aY, aZ, wY, wX, wZ, mX, mY, mZ = old_data_rectangle(path)
    elif self_data == True:
        df = pull_and_sort_data(path)
        ...
        print("Something went wrong in choosing self or not data.")
        exit(-1)
...
def rotate_vector(vector, q):
    ...
    return vRotated
def self(self_data, path, q_scale, tq_scale):
    # setup
    R_initialOrientation = np.eye(3)
    initialPosition = np.zeros(3)
    accMeasured, omega, mag_array, time_array = grab_data_from_df(self_data, path)
    ...
    q = normalize(q * q_scale + tq * tq_scale)
    # q = q * 0.5 + tq * 0.5
...
model = input("Would you like to run the baseline model? (BL) Or the final model? (F): ")
if model == 'BL':
    path = input("BL model only does provided data.  Please enter Path (Eg. Rectangle): ")
    dead_reckon(path)
    exit(0)
self_data = input("Do you wish do use data collected? (Y) Or provided data? (N): ")
if self_data == 'N':
    self_data = False
elif self_data == 'Y':
    self_data = True
    ...
    print("Enter Y or N.")
    exit(-1)
path = input("Enter the name of the shape (Self: B, Provided: Rectangle): ")

q_scale = input("Enter the % calc_quat you would like to use: ")
tq_scale = input("Enger the % tq_scale you would like to use: ")
q_scale = float(q_scale)
tq_scale = float(tq_scale)
if q_scale + tq_scale != 1.0:
    print("q_scale + tq_scale must equal 1.0")
    exit(-1)

q, pos, vel = self(self_data, path, q_scale, tq_scale)
